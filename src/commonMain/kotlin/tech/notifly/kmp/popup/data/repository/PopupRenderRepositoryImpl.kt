...
internal class PopupRenderRepositoryImpl(private val baseUrl: String, private val client: () -> HttpClient) : PopupRenderRepository {
    override suspend fun render(request: ValidatedPopupRenderRequest): PopupRenderResult {
        ...
        return try {
            val endpoint = URLBuilder(origin).apply {
                encodedPath = "/projects/${segment(request.projectId)}/users/${segment(request.notiflyUserId)}/popup-pages/${segment(request.campaignId)}"
            }.build()
            client().preparePost(endpoint) {
                ...
                when (status) {
                    200 -> {
                        ...
                        if (!type.equals("text/html", ignoreCase = true) || html.trimJsWhitespace().isEmpty()) PopupRenderResult.Failed("invalid_response", 200)
                        else PopupRenderResult.Rendered(html)
                    }
                    204 -> PopupRenderResult.Skipped
                    else -> PopupRenderResult.Failed(when (status) {
                        400 -> "invalid_request"
                        404 -> "popup_not_found"
                        408 -> "request_timeout"
    ...
    private fun segment(value: String): String = buildString {
        for (byte in value.encodeToByteArray()) {
            ...
            else { append('%'); append("0123456789ABCDEF"[code ushr 4]); append("0123456789ABCDEF"[code and 15]) }
        }
    }
}
