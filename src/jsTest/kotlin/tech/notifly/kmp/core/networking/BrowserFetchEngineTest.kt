...
class BrowserFetchEngineTest {
    private fun response(status: Int = 200, body: () -> Promise<dynamic> = { Promise.resolve(js("new Uint8Array([104, 105]).buffer")) }): dynamic {
        val value = js("({})")
        value.status = status
        value.statusText = "OK"
    ...
    @Test fun fetchGetsPrivacyOptionsAndEncodedBodyBeforeSending() = runTest {
        var calls = 0
        val client = HttpClient(BrowserFetchEngine { url, init ->
            calls++
    ...
    @Test fun cancellationAbortsWhileBodyPromiseIsStillPending() = runTest {
    ...
    @Test fun cancellationAbortsBeforeResponseHeaders() = runTest {
    ...
    @Test fun nativeJavaScriptRejectionsBecomeNetworkErrorWithoutStatus() = runTest {
        val request = ValidatedPopupRenderRequest("0123456789abcdef0123456789abcdef", "sdk", "campaign", "user", "device", "open", "{}")
