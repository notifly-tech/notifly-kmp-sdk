...
class BrowserHttpIntegrationTest {
    ...
    fun sameAndCrossOriginRequestsOmitCookiesAndUseOnlyAllowedPreflightHeaders() = runTest {
        ...
        try {
            ...
            val sameOriginResponse = client.post("$sameOrigin/__notifly_popup_fixture/html") {
            ...
            val crossOriginResponse = client.post("$crossOrigin/__notifly_popup_fixture/html") {
                contentType(ContentType.Application.Json)
                header("X-Notifly-SDK-Version", "browser-smoke")
                setBody("{}")
            }
            ...
            val preflightHeaders = checkNotNull(crossOriginResponse.headers["X-Observed-Preflight-Headers"]).lowercase()
    ...
    private suspend fun awaitState(client: io.ktor.client.HttpClient, header: String, expected: String) {
        repeat(100) {
            val state = client.post("$sameOrigin/__notifly_popup_fixture/state")
