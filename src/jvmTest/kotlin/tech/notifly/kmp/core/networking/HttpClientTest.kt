...
class HttpClientTest {
    @Test
    fun doesNotInstallKmpTimeoutPolicy() {
        val client = createHttpClient()
        try {
            assertNull(client.pluginOrNull(HttpTimeout))
        } finally {
            client.close()
        }
    }
    ...
    @Test
    fun doesNotFollowRedirectsOrReplayServiceUnavailable() =
        runBlocking {
    ...
    @Test
    fun disconnectDoesNotReplayRequest() =
        runBlocking {
            val server = MockWebServer()
            server.start()
            ...
            try {
                var failed = false
                try {
                    client.post(server.url("/render").toString()) { setBody("private body") }.bodyAsText()
                } catch (
                    error: Exception,
                ) {
                    failed =
    ...
    @Test
    fun responseCookiesAreNotAttachedToLaterCalls() =
        runBlocking {
            val server = MockWebServer()
            server.start()
    ...
    @Test
    fun readsDelayedBodyWithPlatformTimeoutDefaults() =
        runBlocking {
            val server = MockWebServer()
            server.start()
