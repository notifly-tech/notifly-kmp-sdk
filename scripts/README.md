...
| [smoke-swift-consumer.swift](smoke-swift-consumer.swift) | Swift consumer test program. Checks the user ID transition API and popup behavior: the `static` outcome, errors after closing the renderer, cancellation, and duplicate callbacks. This is source code compiled by the shell script, not a standalone executable script. | Used by `smoke-swift-consumer.sh` |
